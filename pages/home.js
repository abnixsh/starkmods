...
function HomePage() {
  ...
  const renderCard = (id, innerHtml) => {
    ...
    // Using the ios-glass-card class defined in the style block below
    return `
  ...
  if (!document.getElementById('stark-hero-v2')) {
    ...
    s.textContent = `
      ...
      .stark-title-reveal { opacity: 0; animation: starkTitleIn 1s ease forwards; animation-delay: 2.2s; }
      @keyframes starkTitleIn { 0% { opacity: 0; transform: scale(0.96); } 100% { opacity: 1; transform: scale(1); } }
      .stark-underline { width: 0; animation: starkLineIn 0.8s ease forwards; animation-delay: 2.8s; }
      @keyframes starkLineIn { to { width: 120px; } }
      .stark-in { opacity: 0; transform: translateY(16px); animation: starkElIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
      @keyframes starkElIn { to { opacity: 1; transform: translateY(0); } }
      .stark-cursor { display: inline-block; width: 3px; height: 1em; background: currentColor; margin-left: 4px; opacity: 0; animation: starkCursorBlink 1s step-end infinite; animation-delay: 2.2s; }
      @keyframes starkCursorBlink { 0%, 100% { opacity: 0; } 50% { opacity: 1; } }
      .stark-shimmer { position: relative; overflow: hidden; }
      .stark-shimmer::after { content: ''; position: absolute; top: 0; left: -100%; width: 50%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent); animation: starkShimmer 4s ease-in-out infinite; animation-delay: 3.5s; pointer-events: none; }
      @keyframes starkShimmer { 0% { left: -100%; } 50% { left: 150%; } 100% { left: 150%; } }
    `;
    document.head.appendChild(s);
  }

  // Generate SVG path for "Stark Mods" handwriting effect (Same as before)
  ...
  <div class="max-w-6xl mx-auto pb-24 animate-fade-in relative px-3 sm:px-6 pt-6">
    ...
    <section class="ios-glass-panel mb-8 text-center p-8 sm:p-12 shadow-2xl isolate overflow-visible">
       ...
       <div class="stark-particle w-2 h-2 bg-blue-400 top-10 left-10 rounded-full blur-[1px]" style="animation-delay:0s"></div>
       <div class="stark-particle w-3 h-3 bg-purple-400 bottom-10 right-10 rounded-full blur-[2px]" style="animation-delay:1.5s"></div>

       <div class="relative z-10 flex flex-col items-center">
          
          <!-- Live Badge (iOS Pill Style) -->
          <div class="stark-in inline-flex items-center gap-2 py-1.5 px-3 rounded-full bg-white/50 dark:bg-black/30 backdrop-blur-md border border-white/20 shadow-sm mb-4" style="animation-delay:0.1s">
          ...
          <!-- Handwriting & Title -->
          <div class="mb-1 transform scale-90 sm:scale-100 origin-center">
            ${svgHandwriting}
          </div>
          <div class="stark-shimmer -mt-4">
            <h1 class="stark-title-reveal stark-title-font text-5xl sm:text-7xl bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent select-none leading-tight pb-2">
              Stark Mods<span class="stark-cursor text-purple-500"></span>
            </h1>
          </div>
          <div class="flex justify-center mt-1 mb-6">
            <div class="stark-underline h-[3px] rounded-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 opacity-80"></div>
          </div>
          ...
          <div class="stark-in flex flex-wrap justify-center gap-4 w-full" style="animation-delay:3.2s">
             
             <!-- Browse Button -->
             <button onclick="document.getElementById('search-mods').focus()" 
                     class="ios-btn group h-12 px-6 rounded-full flex items-center gap-3 text-slate-700 dark:text-white font-semibold">
                <div class="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300">
             ...
             <button onclick="window.open('https://t.me/starkrc20', '_blank')" 
                     class="ios-btn group h-12 px-6 rounded-full flex items-center gap-3 text-blue-600 dark:text-blue-400 font-semibold">
                <div class="w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                   <span class="material-icons text-sm transform group-hover:-rotate-12 transition">telegram</span>
                </div>
                <span>Channel</span>
             </button>
             ...
             <button onclick="window.open('https://discord.gg/KaeHESH9n', '_blank')" 
                     class="ios-btn group h-12 px-6 rounded-full flex items-center gap-3 text-indigo-600 dark:text-indigo-400 font-semibold">
    ...
    <div class="grid grid-cols-4 gap-3 mb-8 px-1">
       <!-- Stats items use Glass Panel style now -->
    ...
    <!-- ===== IMPROVED SEARCH BAR (Floating iOS style) ===== -->
...
window.filterMods = function() {
   ...
   if(foundCount === 0) {
       if(!noResultEl) {
           ...
           // Using glass panel for no-results too
