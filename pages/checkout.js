            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-lg">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="user-name" value="${window.currentUser.displayName}" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl p-4 outline-none text-slate-900 dark:text-white font-bold" readonly>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-500 mb-1">Telegram Username (For Key Delivery)</label>
                        <input type="text" id="user-contact" placeholder="@username or Phone Number" class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl p-4 outline-none focus:border-blue-500 transition text-slate-900 dark:text-white">
        ...
        <div id="step-2" class="step-content hidden">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-lg">
                ...
                <div class="flex gap-4">
                    ...
                    <button onclick="window.goToStep3()" id="step2-next" disabled class="flex-1 py-4 bg-slate-400 text-white rounded-xl font-bold transition cursor-not-allowed">Proceed</button>
                </div>
            </div>
        </div>
        ...
        <div id="step-3" class="step-content hidden">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border-2 border-blue-600 p-8 shadow-2xl relative overflow-hidden text-center">
                ...
                <div class="mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                    ...
                    <span class="text-xs bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-slate-500">Order #${orderId}</span>
                </div>
                ...
                <div class="text-left bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <div class="flex gap-2">
                        <input type="text" id="final-trans-id" placeholder="e.g. 123456789012" class="flex-1 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-600 rounded-lg p-3 font-mono text-sm outline-none focus:border-green-500 transition text-slate-900 dark:text-white">
                        <button onclick="window.submitOrder()" id="btn-final-submit" class="bg-green-600 hover:bg-green-700 text-white px-4 rounded-lg font-bold shadow-lg transition">
                            Verify
                        </button>
                    </div>
                </div>
...
function renderSmartPaymentDetails() {
    ...
    if(method === 'upi') {
        ...
        // This link forces amount in GPay/Paytm
        ...
        box.innerHTML = `
            ...
            <div class="mt-4 bg-slate-100 dark:bg-slate-900 px-4 py-2 rounded-lg inline-flex items-center gap-2 cursor-pointer hover:bg-slate-200 transition" onclick="navigator.clipboard.writeText('${upiID}')">
    ...
    // Add EasyPaisa / PayPal similar blocks here...
    else {
        // Default Fallback
...
function startTimer() {
    ...
    const interval = setInterval(() => {
        ...
        if(time-- <= 0) { clearInterval(interval); display.innerText="Expired"; }
    }, 1000);
}
...
window.submitOrder = async function() {
    ...
    try {
        ...
        // 2. Call Vercel Backend for Telegram
