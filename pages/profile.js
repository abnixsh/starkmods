function ProfilePage() {
    ...
    if (!window.currentUser) {
        // Agar login nahi hai to home bhej do
        setTimeout(() => window.router.navigateTo('/'), 100);
        return '';
    }

    // Load orders after slight delay to ensure DB is ready
    setTimeout(() => loadUserOrders(), 500);

    return `
    <div class="max-w-4xl mx-auto animate-fade-in pb-20">
        <!-- Header -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 mb-8 flex items-center gap-4">
          ...
          snapshot.forEach(doc => {
              ...
              <div class="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-center">
                  ...
                  <div class="text-right">
                      ${statusBadge}
                      ${o.status === 'approved' ? `<button onclick="window.open('https://t.me/AbnixSH')" class="mt-2 bg-blue-600 text-white px-3 py-1 rounded text-xs">Get Key</button>` : ''}
                  </div>
              </div>`;
          });
          list.innerHTML = html;
