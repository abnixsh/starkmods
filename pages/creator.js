...
/* =========================================
   1. GLOBAL HELPERS (Moved to top to fix errors)
   ========================================= */

// Fix: Attached to window to prevent "not defined" errors
window.readFileAsBase64 = function(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
        ...
        else reject(new Error("File read error (Empty)"));
    };
...
function CreatorPlayerPage() {
  ...
  const slider = (id, label, color) => `<div class="mb-4"><div class="flex justify-between mb-2"><label class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">${label}</label><span id="val-${id}" class="text-[10px] font-bold text-${color}-600">50</span></div><input id="${id}" type="range" min="1" max="100" value="50" oninput="document.getElementById('val-${id}').innerText = this.value" class="w-full h-4 bg-slate-200 dark:bg-slate-700 rounded-full appearance-none cursor-pointer accent-${color}-600"></div>`;
    ...
    <div class="max-w-3xl mx-auto pb-24 px-4 pt-6">
      ...
      <div class="app-card p-6 sm:p-8">
        <form onsubmit="window.submitCustomPlayer(event)" class="space-y-6">
          ...
          <div class="bg-white/50 dark:bg-black/20 p-5 rounded-2xl border border-white/20 dark:border-white/5 backdrop-blur-md"><h3 class="text-xs font-black uppercase text-blue-500 mb-4 flex items-center gap-1"><span class="material-icons text-sm">sports_cricket</span> Batting Skill</h3><div class="mb-4"><label class="block text-[10px] font-bold mb-1 text-slate-500 uppercase">Style</label><select id="cp-bat-type" class="form-input w-full text-xs font-bold h-10"><option value="Balanced">Balanced</option><option value="Radical">Radical</option><option value="Brute">Brute</option><option value="Defensive">Defensive</option></select></div>${slider('cp-timing', 'Timing', 'blue')}${slider('cp-aggression', 'Aggression', 'red')}${slider('cp-technique', 'Technique', 'purple')}</div>
          <div id="cp-bowling-section" class="hidden bg-white/50 dark:bg-black/20 p-5 rounded-2xl border border-white/20 dark:border-white/5 backdrop-blur-md"><h3 class="text-xs font-black uppercase text-green-500 mb-4 flex items-center gap-1"><span class="material-icons text-sm">sports_baseball</span> Bowling Skill</h3><div class="grid grid-cols-2 gap-4 mb-4"><div><label class="block text-[10px] font-bold mb-1 text-slate-500 uppercase">Style</label><select id="cp-bowl-type" class="form-input w-full text-xs h-10" onchange="window.updateBowlingActions('cp')"><option value="Fast">Fast</option><option value="Medium">Medium</option><option value="Spin">Spin</option></select></div><div><label class="block text-[10px] font-bold mb-1 text-slate-500 uppercase">Action</label><select id="cp-bowl-action" class="form-input w-full text-xs font-bold h-10"></select></div></div>${slider('cp-bowl-move', 'Movement', 'green')}${slider('cp-bowl-skill', 'Accuracy', 'orange')}</div>
  ...
  return `
    <div class="max-w-4xl mx-auto pb-24 px-4 pt-6">
      ...
      <div class="app-card p-6 mb-6">
        <h2 class="text-sm font-bold uppercase tracking-widest text-slate-400 mb-4">Step 1: Details</h2>
      ...
      <div class="app-card p-6 mb-8">
        <div class="flex justify-between items-center mb-4"><h2 class="text-sm font-bold uppercase tracking-widest text-slate-400">Step 2: Squad</h2><span class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-600 px-2 py-1 rounded font-bold" id="squad-count">0/15</span></div>
        <button onclick="window.openPlayerModal()" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-500/20 mb-4 flex items-center justify-center gap-2"><span class="material-icons">person_add</span> Add Player</button>
...
// --- GENERATE DETAILED SUMMARY FOR BOT (TEXT FALLBACK) ---
...
window.submitCustomTeam = async function() {
    ...
    try {
        ...
        let jB = null, lB = null;
        if(jFile) jB = await window.readFileAsBase64(jFile); if(lFile) lB = await window.readFileAsBase64(lFile);
        ...
        const data = {
            ...
            squadSummary: summary, description: summary, // FOR BOT
...
window.submitCustomJersey = async function (evt) {
  ...
  try {
      ...
      const b64 = await window.readFileAsBase64(f);
