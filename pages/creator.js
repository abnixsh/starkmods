...
function CreatorMenuUI() {
  if (!window.currentUser) return `<div class="p-10 text-center"><h1 class="text-2xl font-bold">Login Required</h1><button onclick="window.googleLogin()" class="btn mt-4 bg-blue-600 text-white px-4 py-2 rounded">Login</button></div>`;
  ...
  return `
    <div class="max-w-5xl mx-auto animate-fade-in pb-24 px-4 pt-6">
      ...
      <div class="grid sm:grid-cols-3 gap-6">
        <button class="app-card p-6 text-left hover:scale-[1.02] transition bg-white dark:bg-slate-800 rounded-xl shadow border border-slate-100 dark:border-slate-700" onclick="window.router.navigateTo('/creator-player')">
        ...
        <button class="app-card p-6 text-left hover:scale-[1.02] transition bg-white dark:bg-slate-800 rounded-xl shadow border border-slate-100 dark:border-slate-700" onclick="window.goToCreatorJersey()">
        ...
        <button class="app-card p-6 text-left hover:scale-[1.02] transition bg-white dark:bg-slate-800 rounded-xl shadow border border-slate-100 dark:border-slate-700" onclick="window.goToCreatorTeam()">
...
/* =========================================
   3. HELPERS (Text Gen & UI)
   ========================================= */

// --- TEXT GENERATOR (Crucial for Bot) ---
function generatePlayerStatsText(p) {
    let bowlText = "   Bowling: N/A";
    if (p.bowlStyle && p.bowlStyle !== 'N/A') {
        bowlText = `   Bowl: ${p.bowlStyle} (${p.bowlHand})\n` +
                   `   Action: ${p.bowlAction} | Mov: ${p.bowlMovement} | Skl: ${p.bowlSkill}`;
    }
...
function renderTeamSelectorHTML(idPrefix) {
  ...
  let dropdownContent = categories.map(cat => {
...
function CreatorPlayerPage() {
  ...
  // NEW: INPUT FIELDS INSTEAD OF SLIDERS
  const numInput = (id, label) => `<div><label class="block text-[10px] font-bold mb-1 text-slate-500 uppercase">${label} (0-100)</label><input id="${id}" type="number" min="0" max="100" class="form-input w-full h-10 text-xs font-bold" placeholder="80"></div>`;
...
function CreatorTeamPage() {
  ...
  const numInputCompact = (id, label) => `<div><label class="text-[9px] font-bold text-slate-500 uppercase mb-1 block">${label}</label><input id="${id}" type="number" min="0" max="100" class="form-input w-full h-8 text-xs font-bold" placeholder="80"></div>`;
  ...
  return `
    <div class="max-w-4xl mx-auto pb-24 px-4 pt-6">
      ...
      <div class="app-card p-6 mb-8">
        <div class="flex justify-between items-center mb-4"><h2 class="text-sm font-bold uppercase text-slate-400">Step 2: Squad</h2><span class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded font-bold" id="squad-count">0/15</span></div>
        <button onclick="window.openPlayerModal()" class="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg mb-4 flex items-center justify-center gap-2"><span class="material-icons">person_add</span> Add Player</button>
    ...
    <div id="player-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-0 sm:p-4">
       ...
       <div class="relative bg-white dark:bg-slate-900 w-full sm:max-w-lg h-full sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-3xl shadow-2xl flex flex-col">
          ...
          <div class="p-5 overflow-y-auto custom-scrollbar space-y-6 pb-20">
             ...
             <select id="tp-type" class="form-input w-full font-bold h-12" onchange="window.updateBowlingOptions('tp')"><option value="Batsman">Batsman</option><option value="Bowler">Bowler</option><option value="All-Rounder">All-Rounder</option><option value="Keeper">Keeper</option></select>
...
window.submitCustomTeam = async function() {
    ...
    try {
        ...
        const jB = j ? await window.readFileAsBase64(j) : null;
        const lB = l ? await window.readFileAsBase64(l) : null;
        
        // BOT TEXT BLOCK
        const summary = window.teamBuilder.players.map(p => generatePlayerStatsText(p)).join('\n\n----------------\n\n');
...
window.submitCustomPlayer = async function (evt) {
  ...
  try {
      ...
      const data = {
        ...
        playerDetails: p, messageForBot: summary, // Send the text
        customFaceBase64: faceDisplay.dataset.isCustom === "true" ? window.tempCustomFaceBase64 : null
      };

      await db.collection('modRequests').add({ ...data, status: 'pending', timestamp: firebase.firestore.FieldValue.serverTimestamp() });
      await window.incrementCreatorUsage();
      try { await fetch('/api/custom-player', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) }); } catch(e){}
      alert("✅ Request Sent!"); window.router.navigateTo('/creator-history');
  } catch(e) { alert(e.message); }
};
