...
// --- TEXT GENERATOR FOR BOT (Crucial) ---
...
function CreatorMenuUI() {
  if (!window.currentUser) return `<div class="p-10 text-center animate-fade-in"><div class="w-20 h-20 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto mb-4 backdrop-blur-md"><span class="material-icons text-4xl text-blue-500">lock</span></div><h1 class="text-2xl font-black mb-2">Login Required</h1><button onclick="window.googleLogin()" class="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-500/30">Login with Google</button></div>`;
  ...
  return `
    <div class="max-w-5xl mx-auto animate-fade-in pb-24 px-4 pt-6">
      <div class="flex justify-between items-center mb-8">
        <div>
           <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Mod Creator</h1>
           <p class="text-slate-500 font-medium">Build your dream game.</p>
        </div>
        <button onclick="window.router.navigateTo('/creator-history')" class="px-4 py-2 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-xs shadow-sm hover:scale-105 transition">
           <span class="material-icons text-sm align-middle">history</span> History
        </button>
      </div>
      ...
      <div class="grid sm:grid-cols-3 gap-6">
        <button class="relative group overflow-hidden p-6 bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-white/5 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 text-left" onclick="window.router.navigateTo('/creator-player')">
           <div class="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-purple-500/10 opacity-0 group-hover:opacity-100 transition"></div>
           <div class="w-14 h-14 bg-blue-100 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mb-4 text-blue-600 dark:text-blue-400 shadow-sm relative z-10"><span class="material-icons text-3xl">person</span></div>
           <div class="relative z-10">
              <div class="font-black text-xl text-slate-900 dark:text-white mb-1">Custom Player</div>
              <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Create detailed players with faces & skills.</p>
           </div>
        </button>

        <button class="relative group overflow-hidden p-6 bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-white/5 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 text-left" onclick="window.goToCreatorTeam()">
           ...
           <div class="w-14 h-14 bg-purple-100 dark:bg-purple-900/30 rounded-2xl flex items-center justify-center mb-4 text-purple-600 dark:text-purple-400 shadow-sm relative z-10"><span class="material-icons text-3xl">groups</span></div>
           <div class="relative z-10">
              <div class="font-black text-xl text-slate-900 dark:text-white mb-1">Custom Team</div>
              <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Build full squads (Pro Only).</p>
           </div>
           <div class="feature-lock-overlay hidden absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-20 rounded-3xl cursor-not-allowed">
              <span class="bg-white/20 backdrop-blur-md border border-white/30 text-white px-3 py-1.5 rounded-full text-[10px] font-bold uppercase flex items-center gap-1"><span class="material-icons text-xs">lock</span> Pro Only</span>
           </div>
        </button>

        <button class="relative group overflow-hidden p-6 bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-white/5 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 text-left" onclick="window.goToCreatorJersey()">
           <div class="absolute inset-0 bg-gradient-to-br from-green-500/10 to-teal-500/10 opacity-0 group-hover:opacity-100 transition"></div>
           <div class="w-14 h-14 bg-green-100 dark:bg-green-900/30 rounded-2xl flex items-center justify-center mb-4 text-green-600 dark:text-green-400 shadow-sm relative z-10"><span class="material-icons text-3xl">checkroom</span></div>
           <div class="relative z-10">
           ...
           <div class="feature-lock-overlay hidden absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-20 rounded-3xl cursor-not-allowed">
              <span class="bg-white/20 backdrop-blur-md border border-white/30 text-white px-3 py-1.5 rounded-full text-[10px] font-bold uppercase flex items-center gap-1"><span class="material-icons text-xs">lock</span> Locked</span>
           </div>
        </button>
      </div>
    </div>`;
...
function renderTeamSelectorHTML(idPrefix) {
  ...
  return `<div class="relative group"><input id="${idPrefix}-team" type="text" readonly placeholder="Select Team" class="form-input w-full h-12 bg-slate-50 dark:bg-slate-800 border-none rounded-xl font-bold cursor-pointer" onclick="document.getElementById('${idPrefix}-team-dropdown').classList.toggle('hidden')"><div id="${idPrefix}-team-dropdown" class="hidden absolute top-full left-0 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-50 mt-2 max-h-60 overflow-y-auto custom-scrollbar">${content}</div></div>`;
}
...
function renderFaceSelectorHTML(idPrefix) {
  return `<div class="bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-slate-700"><div class="flex gap-2 mb-4"><button type="button" onclick="window.switchFaceTab('${idPrefix}', 'preset')" id="${idPrefix}-btn-preset" class="flex-1 py-2 text-[10px] font-bold rounded-lg bg-blue-600 text-white shadow-lg shadow-blue-500/30 transition">Preset</button><button type="button" onclick="window.switchFaceTab('${idPrefix}', 'custom')" id="${idPrefix}-btn-custom" class="flex-1 py-2 text-[10px] font-bold rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition">Custom</button></div><div id="${idPrefix}-view-preset"><input id="${idPrefix}-face-display" type="text" readonly placeholder="Select Face" class="form-input w-full text-xs mb-3 bg-white dark:bg-slate-900 border-none rounded-xl font-bold"><div class="grid grid-cols-4 gap-2 max-h-40 overflow-y-auto custom-scrollbar">${Array.from({length: 80}, (_, i) => i + 1).map(i => `<div onclick="window.selectFace('${idPrefix}', ${i})" class="cursor-pointer border-2 border-transparent hover:border-blue-500 rounded-lg overflow-hidden bg-slate-200 relative"><img src="assets/faces/face_${i}.png" class="w-full aspect-square object-cover" onerror="this.src='https://placehold.co/50?text=${i}'"><div class="absolute bottom-0 right-0 bg-black/60 text-white text-[8px] px-1 font-bold">${i}</div></div>`).join('')}</div></div><div id="${idPrefix}-view-custom" class="hidden text-center py-6 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl"><label class="cursor-pointer text-xs font-bold text-blue-500 flex flex-col items-center gap-2"><span class="material-icons text-3xl">cloud_upload</span><span>Tap to Upload Photo</span><input id="${idPrefix}-face-file" type="file" accept="image/*" class="hidden" onchange="window.handleCustomFaceUpload('${idPrefix}', this)"></label><div id="${idPrefix}-custom-preview" class="text-[10px] text-slate-500 mt-2 truncate font-mono">No file selected</div></div></div>`;
}
window.switchFaceTab = function(p, t) { const pre = document.getElementById(`${p}-view-preset`), cus = document.getElementById(`${p}-view-custom`), btnP = document.getElementById(`${p}-btn-preset`), btnC = document.getElementById(`${p}-btn-custom`); if(t === 'preset') { pre.classList.remove('hidden'); cus.classList.add('hidden'); btnP.className="flex-1 py-2 text-[10px] font-bold rounded-lg bg-blue-600 text-white shadow-lg shadow-blue-500/30 transition"; btnC.className="flex-1 py-2 text-[10px] font-bold rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition"; document.getElementById(`${p}-face-display`).dataset.isCustom="false"; } else { cus.classList.remove('hidden'); pre.classList.add('hidden'); btnC.className="flex-1 py-2 text-[10px] font-bold rounded-lg bg-blue-600 text-white shadow-lg shadow-blue-500/30 transition"; btnP.className="flex-1 py-2 text-[10px] font-bold rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition"; document.getElementById(`${p}-face-display`).dataset.isCustom="true"; } };
...
function CreatorPlayerPage() {
  ...
  const gameBtn = (id, label) => `<button onclick="window.setPlayerGame('${id}')" class="px-5 py-2 rounded-full text-xs font-bold transition shadow-sm ${g === id ? 'bg-blue-600 text-white shadow-blue-500/30' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50'}">${label}</button>`;

  return `
    <div class="max-w-3xl mx-auto pb-24 px-4 pt-6 animate-fade-in">
      <div class="flex justify-between items-center mb-6"><h1 class="text-2xl font-black text-slate-900 dark:text-white">Custom Player</h1><button onclick="window.router.navigateTo('/creator')" class="text-xs font-bold bg-slate-200 dark:bg-slate-700 px-3 py-1 rounded-lg hover:bg-slate-300 transition">Back</button></div>
      <div class="flex gap-2 mb-8 overflow-x-auto pb-2">${gameBtn('rc25', 'RC25')}${gameBtn('rc24', 'RC24')}${gameBtn('rcswipe', 'RC Swipe')}</div>
      <div class="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl p-6 rounded-3xl border border-white/20 dark:border-white/5 shadow-xl">
        <form onsubmit="window.submitCustomPlayer(event)" class="space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6"><div><label class="block text-xs font-bold mb-1 uppercase text-slate-500">Role</label><select id="cp-type" class="form-input w-full h-12 font-bold bg-slate-50 dark:bg-slate-900 border-none rounded-xl" onchange="window.updateBowlingOptions('cp')"><option value="Batsman">Batsman</option><option value="Bowler">Bowler</option><option value="All-Rounder">All-Rounder</option><option value="Keeper">Wicket Keeper</option></select><div class="mt-6"><label class="block text-xs font-bold mb-2 text-slate-500 uppercase">Hands</label><div class="flex gap-3"><select id="cp-bat-hand" class="form-input w-full text-xs h-10 bg-slate-50 dark:bg-slate-900 border-none rounded-xl"><option value="Right">Bat Right</option><option value="Left">Bat Left</option></select><select id="cp-bowl-hand" class="form-input w-full text-xs h-10 bg-slate-50 dark:bg-slate-900 border-none rounded-xl"><option value="Right">Bowl Right</option><option value="Left">Bowl Left</option></select></div></div></div><div><label class="block text-xs font-bold mb-1 uppercase text-slate-500">Face</label>${renderFaceSelectorHTML('cp')}</div></div>
          ...
          <div><label class="block text-[10px] font-bold mb-1 text-slate-400 uppercase">Jersey Number</label><input id="cp-jersey" type="number" class="form-input w-full text-xs h-10 bg-slate-50 dark:bg-slate-900 border-none rounded-xl" placeholder="18"></div>
...
/* =========================================
   6. CUSTOM TEAM PAGE (FULL MOBILE MODAL)
   ========================================= */
...
function CreatorTeamPage() {
  ...
  return `
    <div class="max-w-4xl mx-auto pb-24 px-4 pt-6 animate-fade-in">
      <div class="flex justify-between items-center mb-6"><h1 class="text-2xl font-black text-slate-900 dark:text-white">Team Builder</h1><button onclick="window.router.navigateTo('/creator')" class="text-xs font-bold bg-slate-200 dark:bg-slate-700 px-3 py-1 rounded-lg hover:bg-slate-300 transition">Back</button></div>
      ...
      <button onclick="window.submitCustomTeam()" class="btn w-full py-4 shadow-xl shadow-purple-500/20 text-sm font-black tracking-wide bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-2xl transform active:scale-95 transition">SUBMIT TEAM REQUEST</button>
    </div>
    ...
    <div id="player-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-0 sm:p-4 animate-fade-in">
       ...
       <div class="relative bg-slate-50 dark:bg-slate-950 w-full sm:max-w-lg h-full sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-3xl shadow-2xl flex flex-col">
          ...
          <div class="p-6 overflow-y-auto custom-scrollbar space-y-6 pb-24">
             ...
             <div class="bg-white/50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800"><div class="flex justify-between mb-4"><span class="font-bold text-blue-600 uppercase text-xs">Batting Skill</span><select id="tp-bat-type" class="text-xs bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg font-bold"><option value="Balanced">Balanced</option><option value="Radical">Radical</option><option value="Brute">Brute</option><option value="Defensive">Defensive</option></select></div><div class="grid grid-cols-3 gap-3">${numInputCompact('tp-timing','Timing')}${numInputCompact('tp-aggression','Aggr')}${numInputCompact('tp-technique','Technique')}</div></div>
             <div id="tp-bowling-section" class="hidden bg-white/50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-800"><div class="flex justify-between mb-4"><span class="font-bold text-green-600 uppercase text-xs">Bowling Skill</span><div class="flex gap-2"><select id="tp-bowl-type" class="text-xs bg-slate-100 dark:bg-slate-800 px-2 py-1.5 rounded-lg font-bold" onchange="window.updateBowlingActions('tp')"><option value="Fast">Fast</option><option value="Medium">Med</option><option value="Spin">Spin</option></select><select id="tp-bowl-action" class="text-xs bg-slate-100 dark:bg-slate-800 px-2 py-1.5 rounded-lg font-bold w-24"></select></div></div><div class="grid grid-cols-2 gap-3">${numInputCompact('tp-bowl-move','Movement')}${numInputCompact('tp-bowl-skill','Skill')}</div></div>
          </div>
...
window.renderTeamPlayersList = function() {
    ...
    list.innerHTML = window.teamBuilder.players.map((p, i) => `
        <div class="flex justify-between items-center bg-white dark:bg-slate-800 p-3 rounded-xl mb-2 border border-slate-200 dark:border-slate-700 shadow-sm">
           <div><span class="font-bold text-sm text-slate-900 dark:text-white">${i+1}. ${p.name} <span class="text-slate-500 text-xs font-medium uppercase tracking-wide ml-1">${p.role}</span></span></div>
...
window.submitCustomPlayer = async function (evt) {
  ...
  try {
      ...
      const data = {
        ...
        playerDetails: p, playerSummary: summary, // FOR BOT
...
function CreatorJerseyPage() { if (!window.currentUser) { setTimeout(()=>window.router.navigateTo('/creator'),50); return ''; } return `<div class="max-w-3xl mx-auto pb-24 px-4 pt-6"><div class="flex justify-between mb-6"><h1 class="text-2xl font-black">Custom Jersey</h1><button onclick="window.router.navigateTo('/creator')" class="text-xs font-bold bg-slate-200 px-3 py-1 rounded">Back</button></div><div class="app-card p-6 sm:p-8"><form onsubmit="window.submitCustomJersey(event)" class="space-y-6"><div><label class="block text-xs font-bold mb-2 uppercase">Team Name</label>${renderTeamSelectorHTML('cj')}</div><div><label class="block text-xs font-bold mb-2 uppercase">Jersey Texture (PNG)</label><input id="cj-file" type="file" accept="image/*" class="text-xs w-full"></div><button type="submit" class="btn w-full py-4 shadow-lg text-sm bg-green-600 text-white">Submit Jersey</button></form></div></div>`; }
function CreatorHistoryPage() { if (!window.currentUser) { setTimeout(()=>window.router.navigateTo('/'),50); return ''; } if (window.historyUnsubscribe) window.historyUnsubscribe(); setTimeout(()=>{ const c=document.getElementById('creator-history'); if(!c) return; window.historyUnsubscribe = db.collection('modRequests').where('userId','==',window.currentUser.uid).onSnapshot(s=>{ if(s.empty) { c.innerHTML='<div class="text-center py-8 text-slate-400">No requests.</div>'; return; } const docs = []; s.forEach(d => docs.push(d.data())); docs.sort((a,b)=>(b.timestamp?.toMillis?.()||0)-(a.timestamp?.toMillis?.()||0)); c.innerHTML=docs.map(r=>`<div class="app-card p-4 flex justify-between items-center"><div class="text-sm font-bold">${r.type==='team'?r.teamName:r.playerName||r.teamName}</div><div class="text-xs font-bold text-amber-500">${r.status === 'pending' ? 'Pending (Wait < 24h)' : r.status}</div></div>`).join(''); }); },100); return `<div class="max-w-4xl mx-auto pb-24 px-4 pt-6"><h1 class="text-2xl font-black mb-6">History</h1><div id="creator-history" class="space-y-4"></div></div>`; }
