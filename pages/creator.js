...
function CreatorTeamPage() {
  ...
  const numInputCompact = (id, label) => `<div><label class="text-[9px] font-bold text-slate-500 uppercase mb-1 block">${label}</label><input id="${id}" type="number" min="0" max="100" class="form-input w-full h-10 text-xs font-bold" placeholder="80"></div>`;
...
window.submitCustomTeam = async function() {
    ...
    try {
        ...
        // --- PRE-FORMATTED TEXT FOR BOT (FIXED) ---
        ...
        const data = {
            ...
            botMessage: summary, // Use this key in your bot
...
window.submitCustomPlayer = async function (evt) {
  ...
  try {
      ...
      const data = {
        ...
        playerDetails: p, botMessage: summary, // Use this key
