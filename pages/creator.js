...
function generateSummaryText(p) {
    let txt = `Name: ${p.name}\n` +
              ...
              `Face: ${p.face}\n` +
              `Bat Hand: ${p.battingHand}\n` +
              `Bowl Hand: ${p.bowlingHand}\n` +
              `Bat Style: ${p.batsmanType}\n` +
              `Stats: T-${p.timing} A-${p.aggression} Tec-${p.technique}`;
    
    if (['bowler', 'all-rounder'].includes(p.role)) {
        txt += `\nBowl Style: ${p.bowlerType}\nAction: ${p.bowlingAction}\nSkill: ${p.bowlingSkill} | Move: ${p.bowlingMovement}`;
    }
    return txt;
}
    ...
    <div class="max-w-5xl mx-auto animate-fade-in pb-24 px-4 pt-6">
      ...
      <div class="grid sm:grid-cols-3 gap-6">
        <button class="app-card p-6 text-left hover:scale-[1.02] transition" onclick="window.router.navigateTo('/creator-player')">
           <div class="text-4xl mb-2">👤</div><div class="font-bold text-lg">Custom Player</div>
        </button>
        <button class="app-card p-6 text-left hover:scale-[1.02] transition" onclick="window.goToCreatorTeam()">
           <div class="text-4xl mb-2">👥</div><div class="font-bold text-lg">Custom Team</div>
        </button>
        <button class="app-card p-6 text-left hover:scale-[1.02] transition" onclick="window.goToCreatorJersey()">
           <div class="text-4xl mb-2">👕</div><div class="font-bold text-lg">Custom Jersey</div>
        </button>
      </div>
    </div>`;
}
...
function renderTeamSelectorHTML(idPrefix) {
  ...
  let content = categories.map(cat => {
      if(Array.isArray(TEAMS_DATA[cat])) {
          return `<div class="border-b border-slate-100 dark:border-slate-700"><div class="px-4 py-2 bg-slate-50 dark:bg-slate-700/50 text-[10px] font-bold uppercase text-slate-500 sticky top-0">${cat}</div>${TEAMS_DATA[cat].map(t => `<div onclick="window.selectTeam('${idPrefix}', '${t}')" class="px-4 py-2 text-xs cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-700">${t}</div>`).join('')}</div>`;
      } else {
          return Object.keys(TEAMS_DATA[cat]).map(sub => `<div class="border-b border-slate-100 dark:border-slate-700"><div class="px-4 py-2 bg-slate-50 dark:bg-slate-700/50 text-[10px] font-bold uppercase text-slate-500 sticky top-0">${sub}</div>${TEAMS_DATA[cat][sub].map(t => `<div onclick="window.selectTeam('${idPrefix}', '${t}')" class="px-4 py-2 text-xs cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-700">${t}</div>`).join('')}</div>`).join('');
      }
  }).join('');
...
window.switchFaceTab = function(p, t) { const pre = document.getElementById(`${p}-view-preset`), cus = document.getElementById(`${p}-view-custom`); if(t === 'preset') { pre.classList.remove('hidden'); cus.classList.add('hidden'); document.getElementById(`${p}-face-display`).dataset.isCustom="false"; } else { cus.classList.remove('hidden'); pre.classList.add('hidden'); document.getElementById(`${p}-face-display`).dataset.isCustom="true"; } };
window.selectFace = function(p, id) { const d = document.getElementById(`${p}-face-display`); d.value = `${id}`; d.dataset.faceId = id; };
window.handleCustomFaceUpload = async function(p, inp) { if(inp.files[0]) { try { const b64 = await window.readFileAsBase64(inp.files[0]); document.getElementById(`${p}-custom-preview`).innerText = inp.files[0].name; document.getElementById(`${p}-face-display`).value = "Custom"; if(p === 'cp') window.tempCustomFaceBase64 = b64; else inp.dataset.tempB64 = b64; } catch(e) { alert(e.message); } } };
...
window.addTeamPlayer = function () {
  ...
  // LOWERCASE VALUES FOR BOT COMPATIBILITY
  const p = {
      name: name, role: role, // role is now 'batsman' (lowercase)
      ...
      battingHand: document.getElementById('tp-bat-hand').value, // 'right' or 'left'
      bowlingHand: document.getElementById('tp-bowl-hand').value,
      
      batsmanType: document.getElementById('tp-bat-type').value, // 'balanced', 'radical'
      timing: document.getElementById('tp-timing').value || '80',
      aggression: document.getElementById('tp-aggression').value || '80',
      technique: document.getElementById('tp-technique').value || '80',
      
      bowlerType: isBowler ? document.getElementById('tp-bowl-type').value : 'N/A',
      bowlingAction: isBowler ? document.getElementById('tp-bowl-action').value : 'N/A',
      bowlingMovement: isBowler ? document.getElementById('tp-bowl-move').value || '80' : '0',
      bowlingSkill: isBowler ? document.getElementById('tp-bowl-skill').value || '80' : '0'
  };
...
window.submitCustomTeam = async function() {
    ...
    try {
        ...
        // Use the old-school player array as expected by your uploaded 'custom-team.js'
        // Your backend iterates 'players' array.
        ...
        const data = {
            ...
            mode: 'new', 
            players: window.teamBuilder.players, // Contains matching keys: playerType, jerseyNumber, etc.
...
window.submitCustomPlayer = async function (evt) {
  ...
  try {
      ...
      const p = {
          ...
          playerType: document.getElementById('cp-type').value, // lowercase
          ...
          // Hands
          battingHand: document.getElementById('cp-bat-hand').value, // lowercase
          bowlingHand: document.getElementById('cp-bowl-hand').value, // lowercase
          
          // Batting
          batsmanType: document.getElementById('cp-bat-type').value, // lowercase
          timing: document.getElementById('cp-timing').value || '80',
          aggression: document.getElementById('cp-aggression').value || '80',
          technique: document.getElementById('cp-technique').value || '80',
          
          // Bowling
          bowlerType: isBowler ? document.getElementById('cp-bowl-type').value : 'N/A',
          bowlingAction: isBowler ? document.getElementById('cp-bowl-action').value : 'N/A',
          bowlingSkill: isBowler ? document.getElementById('cp-bowl-skill').value || '80' : '0'
      };
      ...
      const data = {
        ...
        ...p, // Spread the correctly named properties
        customFaceBase64: faceDisplay.dataset.isCustom === "true" ? window.tempCustomFaceBase64 : null,
        useCustomFace: faceDisplay.dataset.isCustom === "true"
      };
