...
// --- TEXT GENERATOR (Fixes "Undefined" on Bot) ---
function generateBotText(p, index = 1) {
...
// --- NEW INPUT FIELD COMPONENT (0-100) ---
const numInput = (id, label) => `<div><label class="block text-[10px] font-bold mb-1 text-slate-500 uppercase">${label}</label><input id="${id}" type="number" min="0" max="100" class="form-input w-full h-10 text-xs font-bold" placeholder="80"></div>`;
    ...
    <div id="player-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-0 sm:p-4">
       ...
       <div class="relative bg-white dark:bg-slate-900 w-full sm:max-w-lg h-full sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-3xl shadow-2xl flex flex-col">
          ...
          <div class="p-5 overflow-y-auto custom-scrollbar space-y-6 pb-20">
             ...
             <div class="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl"><div class="flex justify-between mb-4"><span class="font-bold text-blue-600">BATTING</span><select id="tp-bat-type" class="text-xs bg-white dark:bg-slate-700 px-2 py-1 rounded font-bold"><option value="Balanced">Balanced</option><option value="Radical">Radical</option><option value="Brute">Brute</option><option value="Defensive">Defensive</option></select></div><div class="grid grid-cols-3 gap-2">${numInput('tp-timing','Timing')}${numInput('tp-aggression','Aggr')}${numInput('tp-technique','Technique')}</div></div>
             <div id="tp-bowling-section" class="hidden bg-slate-50 dark:bg-slate-800 p-4 rounded-xl"><div class="flex justify-between mb-4"><span class="font-bold text-green-600">BOWLING</span><div class="flex gap-1"><select id="tp-bowl-type" class="text-xs bg-white dark:bg-slate-700 px-1 py-1" onchange="window.updateBowlingActions('tp')"><option value="Fast">Fast</option><option value="Medium">Medium</option><option value="Spin">Spin</option></select><select id="tp-bowl-action" class="text-xs bg-white dark:bg-slate-700 px-1 py-1 w-20"></select></div></div><div class="grid grid-cols-2 gap-2">${numInput('tp-bowl-move','Movement')}${numInput('tp-bowl-skill','Skill')}</div></div>
          </div>
...
/* =========================================
   8. SUBMISSION LOGIC (BOT PROOFED)
   ========================================= */
...
window.submitCustomTeam = async function() {
    ...
    try {
        ...
        const summary = window.teamBuilder.players.map((p, i) => generateBotText(p, i+1)).join('\n\n----------------\n\n');
...
window.submitCustomPlayer = async function (evt) {
  ...
  try {
      ...
      const summary = generateBotText(p, 1);
      ...
      const data = {
        ...
        playerDetails: p, messageForBot: summary,
